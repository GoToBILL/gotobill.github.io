...
export const activities = [
  ...
  { period: "2025.02 — 2025.09", title: "Prography 10기", descriptor: "Prography · IT 연합동아리", logo: "/images/organizations/prography.png", logoAlt: "Prography", logoClass: "prography", role: "10기 · 백엔드 엔지니어" },
  { period: "2025.03 — 2025.12", title: "소프트웨어 마에스트로 16기", descriptor: "SW Maestro", logo: null, logoAlt: "소프트웨어 마에스트로", logoClass: "sw-maestro", role: "백엔드 엔지니어" },
]
...
export const projects = [
  {
    ...
    story: [
      ...
      { title: "결과와 배운 점", paragraphs: ["처리 위치와 전송 상태가 분리되면서 실패 범위와 재처리 대상을 추적할 수 있게 됐습니다.", "과금 흐름의 신뢰성은 재시도 횟수보다 ‘무엇을 이미 했는지’ 남기는 구조에서 시작한다는 점을 확인했습니다."] },
    ],
    diagram: {
  ...
  },
  {
    ...
    story: [
      ...
      { title: "결과와 배운 점", paragraphs: ["현재는 비운영 환경의 실제 데이터로 검증하는 단계입니다. 화면을 열 때 수집하지 않는 구조와 상태 판정 규칙의 일관성을 확인했습니다.", "관측 시스템은 더 많은 신호보다 신호의 시점과 충돌 규칙을 먼저 정의해야 설명 가능한 결과를 준다는 점을 배웠습니다."] },
    ],
    diagram: {
  ...
  },
  {
    ...
    story: [
      ...
      { title: "결과와 배운 점", paragraphs: ["지역 차이는 진입 지점에서 처리되고 감사 로직은 확인된 사용자·접속 정보만 다루게 됐습니다.", "헤더의 이름보다 그 값을 누가 만들었고 어느 경로를 통과했는지가 감사 데이터의 신뢰성을 결정합니다."] },
    ],
    diagram: {
  ...
  },
  {
    ...
    story: [
      ...
      { title: "결과와 배운 점", paragraphs: ["데이터를 재생성하지 않고 컬럼을 추가하면서 과거 스냅샷을 읽는 규칙을 만들었습니다.", "다만 일부 카탈로그 읽기 경로는 추가 확인이 필요합니다. 스키마 변경은 쓰기 성공뿐 아니라 모든 읽기 경로의 호환성까지 확인해야 끝난다는 점을 남겼습니다."] },
    ],
    diagram: {
  ...
  },
  {
    slug: "ai-harness",
    group: "NHN Cloud",
    title: "AI Native 개발 체계 구축",
    headline: "자동화를 더 쌓는 대신, 실제로 쓰이는 규칙만 남겼습니다",
    status: "진행 중",
    summary: "AI 개발 자동화의 복잡도를 실제 사용 기록으로 측정하고 검증 중심 흐름으로 단순화했습니다.",
    stack: ["AI 개발 자동화", "GitHub Actions", "Jenkins", "Playwright", "Docker"],
    opening: ["AI 개발 자동화를 도입한 뒤 규칙과 훅, 지시 파일이 계속 늘었습니다. 자동화를 더한 만큼 결과가 좋아지는지 알기 어려웠고, 개발자가 구조를 이해하는 비용도 커졌습니다.", "기능 수가 아니라 실제 사용 기록을 기준으로 남길 규칙을 골랐습니다. 입력·산출물·완료 조건, 자동 검사, 테스트 결과와 변경 내역, 사람 승인이라는 최소 흐름으로 다시 정리했습니다."],
    story: [
      { title: "문제의 시작", paragraphs: ["큰 요청을 한 번에 맡기고 완료 선언을 믿으면 변경 범위와 검증 근거가 섞였습니다. 실패 원인을 찾기 위해 자동화 자체를 디버깅하는 시간이 늘었습니다."] },
      { title: "제약 조건", paragraphs: ["개발 환경과 운영 환경의 경계를 넘지 않아야 했고, 자동화가 만든 변경도 사람이 재현할 수 있는 검사 결과를 남겨야 했습니다."] },
      { title: "설계 선택", paragraphs: ["요청에 입력·산출물·완료 조건을 명시하고 작은 단위로 나눴습니다. AI 결과는 정적 검사, 단위 테스트와 E2E를 거쳐 테스트 결과와 변경 내역을 함께 전달합니다.", "마지막 승인 권한은 사람에게 남겼습니다. 자동화는 결정을 대신하지 않고 판단할 근거를 모으는 역할에 집중했습니다."] },
      { title: "실패를 막는 장치", paragraphs: ["검사에 실패하면 전체 작업을 반복하지 않고 실패한 범위만 제한해 다시 시도합니다. 환경을 바꾸는 작업은 별도 승인 지점에서 멈추도록 했습니다."] },
      { title: "검증", paragraphs: ["실제 사용 기록을 기준으로 규칙별 호출과 유지 비용을 비교했습니다. 정적 검사, 단위 테스트와 E2E가 같은 완료 기준을 가리키는지도 반복 확인했습니다."] },
      { title: "결과와 배운 점", paragraphs: ["상시 로드 규칙 문서는 385줄에서 88줄로 77% 줄였고, 리뷰 자동화 문서는 591줄에서 210줄로 줄였습니다. 전체 코드는 4,524줄 순감소했습니다.", "자동화의 품질은 규칙의 개수가 아니라 실패를 드러내는 검사와 사람이 이해할 수 있는 승인 지점에서 나온다는 결론을 얻었습니다."] },
    ],
    diagram: {
  ...
  },
  {
    ...
    story: [
      ...
      { title: "배운 점", paragraphs: ["같은 전문 검색 인덱스도 데이터 크기와 필터 순서에 따라 전혀 다른 비용을 냅니다. 인덱스를 더 붙이기 전에 사용자가 실제로 읽는 범위를 데이터 구조에 반영하는 것이 더 큰 효과를 냈습니다.", "쿼리 튜닝의 각 시도를 지표로 남겼기 때문에 어느 지점에서 구조 변경이 필요한지 판단할 수 있었습니다. 관측은 결과를 보여주는 장식이 아니라 다음 설계 결정을 위한 입력이었습니다."] },
    ],
    diagram: {
  ...
  },
  {
    ...
    title: "Runtale",
    status: "팀 프로젝트 · 완료",
    summary: "거리·페이스 기반 러닝 티어 로직을 구현하고 이야기 진행 흐름에 연결했습니다.",
    stack: ["Spring Boot", "MySQL", "AWS", "GitHub Actions", "Nginx"],
    context: "러닝 기록을 이야기와 성장 경험으로 연결하고자 했습니다.",
    problem: "거리와 페이스의 경계값에서 러닝 티어가 일관되게 판정되어야 했습니다.",
    decision: "거리·페이스 기반 티어 판정 규칙을 구현하고 이야기 진행 흐름에 연결했습니다.",
    reliability: "등급 경계값 시나리오를 테스트해 판정 규칙을 확인했습니다.",
    validation: "거리·페이스 경계값 시나리오를 테스트하고 CI/CD 배포 흐름을 확인했습니다.",
    outcome: "러닝 티어 로직과 이야기 진행의 연결을 구현했습니다.",
    diagram: {
      title: "Runtale 기록 처리 흐름",
      mermaid: `flowchart LR
        A["러닝 시작"] --> B["러닝 기록 수집"] --> C["거리 · 페이스 계산"]
        C --> D["경계값 판정"] --> E["등급 결정"]:::current --> F["이야기 진행"]`,
      nodes: ["사용자", "활동 API", "페이스 · 거리 규칙", "등급 · 이야기", "기록"],
      secondary: ["GitHub Actions", "AWS", "Nginx"],
    },
  },
]
