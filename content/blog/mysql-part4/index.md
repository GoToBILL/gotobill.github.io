...
### 언두 로그 vs 리두 로그: 왜 버퍼 관리가 다른가?

**리두 로그**:
- 변경사항(delta)만 저장 → 크기가 작음
- 순차적으로 빠르게 기록해야 함 (WAL 원칙)
- 전용 버퍼(리두 로그 버퍼) → 전용 파일에 순차 쓰기

**언두 로그**:
- 변경 전 전체 데이터 저장 → 크기가 큼
- 트랜잭션마다 랜덤하게 접근 (롤백, MVCC 읽기)
- 버퍼 풀에서 관리 → 다른 데이터 페이지처럼 캐싱/플러시

언두 로그도 결국 **페이지 단위**로 관리되기 때문에, 버퍼 풀에서 다른 데이터 페이지처럼 캐싱하고 플러시하는 것이 더 효율적입니다. 별도 버퍼를 만들면 중복 관리 오버헤드만 발생합니다.

### 언두 로그 디스크 기록 시점

언두 로그는 버퍼 풀의 다른 더티 페이지와 동일하게 처리됩니다.

**디스크에 기록되는 시점**:
1. **체크포인트 발생 시**: 백그라운드 스레드가 주기적으로 더티 페이지를 디스크에 기록
2. **버퍼 풀 공간 부족 시**: LRU 알고리즘에 의해 오래된 페이지 방출
3. **플러시 리스트 플러시**: 백그라운드 페이지 클리너 스레드가 처리

**리두 로그와 비교**:
```
리두 로그: COMMIT 시점에 반드시 디스크에 기록 (동기적)
          → 크래시 복구를 위해 필수

언두 로그: 적절한 시점에 백그라운드에서 기록 (비동기적)
          → 급하지 않음, 버퍼 풀이 관리
```

### 커밋 후에도 언두 로그가 필요한 이유

트랜잭션이 커밋됐다고 언두 로그를 바로 삭제할 수 없습니다. **MVCC** 때문입니다.

```sql
트랜잭션 A: UPDATE users SET name = 'John' WHERE id = 1;
트랜잭션 A: COMMIT;  ← 커밋됨

트랜잭션 B: SELECT * FROM users WHERE id = 1;
           -- 트랜잭션 B가 A 커밋 전에 시작했다면?
           -- REPEATABLE READ에서는 이전 값을 봐야 함
           -- → 언두 로그 필요!
```

그래서 퍼지 스레드가 "이 언두 로그를 참조하는 트랜잭션이 아무도 없다"고 확인되면 그때 삭제합니다.
...
퍼지 스레드는 필요 없어진 언두 로그를 **삭제**하는 역할입니다. 정확히는 "삭제"보다는 **재사용 가능 상태로 표시**합니다.

```
언두 테이블스페이스 구조:

[슬롯1: 트랜잭션A 언두] → 참조 없음 → 재사용 가능으로 표시
[슬롯2: 트랜잭션B 언두] → 아직 참조됨 → 유지
[슬롯3: 빈 슬롯]
[슬롯4: 트랜잭션C 언두] → 참조 없음 → 재사용 가능으로 표시
```

**Purge vs Truncate 차이**:

| 작업 | 설명 |
|------|------|
| Purge | 슬롯을 재사용 가능으로 표시 (파일 크기 유지) |
| Truncate | 실제로 파일 크기를 줄여서 OS에 공간 반환 |

`innodb_undo_log_truncate`를 활성화해야 디스크 공간이 실제로 줄어듭니다. 안 하면 슬롯만 비워지고 파일 크기는 그대로입니다.
...
리두 로그는 순환 구조이기 때문에 체크포인트가 진행되면 오래된 로그 영역을 새 로그로 덮어씁니다.

체크포인트는 "현재 데이터 파일 기준 크래시 복구"를 보장하지만, 

**과거 백업본에서 특정 시점까지 복원**하려면 백업 이후의 모든 리두 로그가 필요합니다.

예를 들어 3일 전 백업본으로 현재 상태를 복원하려면:

```
[3일 전 백업] + [3일간의 리두 로그] = 현재 상태 복원
                    ↑
              체크포인트로 이미 덮어씌워져서 없음
```

리두 로그 아카이빙은 이 문제를 해결합니다.
